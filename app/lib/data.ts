...
export async function fetchCardData() {
    ...
    `;
        ...
        return {
            ...
            totalPaidInvoices: formatCurrency(result.paid ?? 0),
            totalPendingInvoices: formatCurrency(result.pending ?? 0),
        };
    } catch (error) {
        console.error('Database Error:', error);
...
export async function updateInvoice(
    id: string,
    data: { customerId: string; amount: number; status: 'pending' | 'paid' },
) {
    const amountInCents = data.amount * 100;
